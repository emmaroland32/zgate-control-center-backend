...
import com.zgate.nexus.domain.Webhook;
import com.zgate.nexus.domain.WebhookDelivery;
import com.zgate.nexus.exception.NexusException;
import com.zgate.nexus.repository.ApiKeyRepository;
import com.zgate.nexus.repository.IntegrationConfigRepository;
import com.zgate.nexus.repository.WebhookDeliveryRepository;
import com.zgate.nexus.repository.WebhookRepository;
...
public class IntegrationService {
    ...
    public Webhook updateWebhook(UUID id, Webhook patch) {
        Webhook existing = webhookRepo.findById(id)
                .orElseThrow(() -> new NexusException("Webhook not found: " + id));
    ...
    public void deleteWebhook(UUID id) {
        if (!webhookRepo.existsById(id)) {
            throw new NexusException("Webhook not found: " + id);
        }
    ...
    public List<WebhookDelivery> getWebhookLogs(UUID webhookId) {
        if (!webhookRepo.existsById(webhookId)) {
            throw new NexusException("Webhook not found: " + webhookId);
        }
    ...
    public void revokeApiKey(UUID id, String revokedBy) {
        ApiKey key = apiKeyRepo.findById(id)
                .orElseThrow(() -> new NexusException("API key not found: " + id));
        if (key.isRevoked()) {
            throw new NexusException("API key is already revoked: " + id);
        }
        key.setRevoked(true);
    ...
    public IntegrationConfig toggleIntegration(UUID id) {
        IntegrationConfig config = integrationConfigRepo.findById(id)
                .orElseThrow(() -> new NexusException("Integration not found: " + id));
    ...
    private String sha256Hex(String input) {
        try {
            ...
            throw new NexusException("SHA-256 algorithm not available", e);
        }
    }
